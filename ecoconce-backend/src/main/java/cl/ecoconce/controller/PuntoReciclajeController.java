...
public class PuntoReciclajeController {
    ...
    // Estados de punto para los formularios de gestión (admin) y detalle (mantenedor).
    // Reemplaza /api/bd/estado-punto con el mismo formato JSON.
    @PreAuthorize("hasAnyRole('ADMIN', 'MANTENEDOR')")
    @Transactional(readOnly = true)
    @GetMapping("/estados")
    public List<java.util.Map<String, Object>> estados() {
        return estadoRepository.findAll().stream().map(estado -> {
            java.util.Map<String, Object> fila = new java.util.LinkedHashMap<>();
            fila.put("id", estado.getId());
            fila.put("nombre", estado.getNombre());
            return fila;
        }).toList();
    }
