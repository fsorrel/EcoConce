...
export function ManagePoints() {
  ...
  const [pointToToggle, setPointToToggle] = useState<PuntoReciclaje | null>(null);
  const [toggleAction, setToggleAction] = useState<"activar" | "desactivar" | null>(null);

  const desactivarPunto = (punto: PuntoReciclaje) => {
    setPointToToggle(punto);
    setToggleAction("desactivar");
  };

  const activarPunto = (punto: PuntoReciclaje) => {
    setPointToToggle(punto);
    setToggleAction("activar");
  };

  const handleConfirmToggle = async () => {
    if (!pointToToggle || !toggleAction) return;

    const punto = pointToToggle;
    const action = toggleAction;

    setPointToToggle(null);
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : `No se pudo ${action} el punto`);
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="p-8 space-y-8 bg-[#f5f7f5]">
      ...
      <AlertDialog open={pointToToggle !== null} onOpenChange={() => { setPointToToggle(null); setToggleAction(null); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {toggleAction === "desactivar" ? "¿Desactivar punto de reciclaje?" : "¿Activar punto de reciclaje?"}
            </AlertDialogTitle>
            <AlertDialogDescription>
              {toggleAction === "desactivar" 
                ? `¿Seguro que deseas desactivar "${pointToToggle?.nombre}"? Los ciudadanos dejarán de verlo en el mapa hasta que sea reactivado.`
                : `¿Seguro que deseas activar "${pointToToggle?.nombre}"? El punto volverá a ser visible y editable para los ciudadanos.`
              }
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancelar</AlertDialogCancel>
            <AlertDialogAction onClick={handleConfirmToggle} className={toggleAction === "desactivar" ? "bg-red-600 hover:bg-red-700 text-white" : "bg-[#3d5a47] hover:bg-[#2d4437] text-white"}>
              {toggleAction === "desactivar" ? "Desactivar" : "Activar"}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
