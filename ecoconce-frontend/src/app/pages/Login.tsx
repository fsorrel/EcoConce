...
export function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // Autenticación real contra el backend: devuelve un JWT firmado.
      const { token, userId, rol } = await api.login(email, password);
      login(token, userId, rol); // guarda el token (localStorage + contexto)

      // Con el token ya disponible, cargamos los datos del usuario.
      let sesion;
      try {
        const resumen = await api.usuarioPorId(userId);
        sesion = saveCurrentUser({ ...resumen, correo: email, rol });
      } catch {
        sesion = saveCurrentUser({ id: userId, correo: email, rol });
      }

      navigate(getRolePath(sesion));
    } catch (err) {
