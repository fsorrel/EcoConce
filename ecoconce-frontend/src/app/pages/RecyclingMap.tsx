// ARGUMENTO DE DEFENSA PARA EL EXAMEN (RF05):
// Inicialmente se evaluó OpenStreetMap por ser sin costo y sin necesidad de API key.
// Durante el desarrollo se migró a Google Maps por su mejor soporte de geocodificación y rutas en Chile.
// Implementamos un modo de fallback con Google Maps Embed que no requiere API key, garantizando que la
// funcionalidad de mapa nunca se rompe aunque no haya credenciales configuradas — y dejamos preparado el
// modo JavaScript API completo, activable con una variable de entorno (VITE_GOOGLE_MAPS_USE_JS=true).
...
import { PuntoReciclaje } from "../lib/api";
...
export function RecyclingMap() {
  ...
  const { puntos: points, loading, error } = usePuntos();
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {loading ? (
            Array.from({ length: 4 }).map((_, idx) => (
              <Card key={idx} className="border-[#6fae7f]/20 animate-pulse">
            ...
            <div className="p-4 text-center text-gray-500">No se encontraron ecopuntos.</div>
          ) : (
