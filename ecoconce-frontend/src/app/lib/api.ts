...
const TOKEN_KEY = "ecoconce_token";
...
export const api = {
  login: (correo: string, contrasena: string) =>
    apiFetch<{ token: string; userId: number; rol: string }>("/api/usuarios/login", {
      method: "POST",
      body: JSON.stringify({ correo, contrasena }),
    }),

  usuarioPorId: (id: number) => apiFetch<UsuarioResumen>(`/api/usuarios/${id}`),
  ...
  me: () => apiFetch<UsuarioAdmin>("/api/usuarios/me"),
  ...
  regiones: () => apiFetch<RegionRow[]>("/api/usuarios/regiones"),

  comunas: () => apiFetch<ComunaRow[]>("/api/usuarios/comunas"),

  estadosPunto: () => apiFetch<BdRow[]>("/api/puntos/estados"),

  // Todos los formularios (solo ADMIN)
  formulariosAdminTodos: () => apiFetch<BdRow[]>("/api/formularios/admin/todos"),

  // Formularios y detalles del propio usuario
  formulariosUsuario: (usuarioId: number) =>
    apiFetch<BdRow[]>(`/api/formularios/usuario/${usuarioId}`),

  detallesUsuario: (usuarioId: number) =>
    apiFetch<BdRow[]>(`/api/formularios/usuario/${usuarioId}/detalles`),
...
export async function refreshCurrentUserFromBackend() {
  ...
  try {
    // Endpoint seguro: devuelve solo los datos del propio usuario autenticado,
    // sin descargar la lista completa de usuarios.
    const me = await api.me();
    const updated = normalizeUsuario(me as unknown as Record<string, unknown>);
