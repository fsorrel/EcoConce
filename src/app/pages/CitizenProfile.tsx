...
export function CitizenProfile() {
  ...
  useEffect(() => {
    ...
    Promise.all([api.formularios(), api.detalleFormularios(), api.canjesAdmin()])
            ...
            <CardContent>
              <div className="space-y-3">
                {medals.map((medal, index) => (
                  <div key={index} className={`flex items-start gap-3 p-3 rounded-lg border border-transparent transition ${medal.obtained ? "bg-[#f5f7f5] border-green-100" : "bg-gray-50 opacity-60"}`}>
