...
import { api, findUsuarioByEmail, saveCurrentUser } from "../lib/api";
...
export function Register() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      try {
        const loginResponse = await api.login(correoNormalizado, formData.contrasena);
        login(loginResponse.token, loginResponse.id, loginResponse.rol);
        saveCurrentUser(loginResponse);
        navigate("/ciudadano");
      } catch (loginErr) {
        navigate("/login");
      }
