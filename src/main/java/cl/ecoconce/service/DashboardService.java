...
public class DashboardService {
    ...
    public DashboardDto obtenerDashboard(Long usuarioId) {
        ...
        if (auth != null) {
            ...
            if (!isAdmin) {
                Usuario loggedInUser = usuarioRepository.findByCorreo(auth.getName())
                        .orElseThrow(() -> new org.springframework.security.access.AccessDeniedException("Usuario autenticado no encontrado"));
                if (!loggedInUser.getId().equals(usuarioId)) {
                    throw new org.springframework.security.access.AccessDeniedException("No tienes permisos para ver el dashboard de otro usuario");
                }
            }
        }
